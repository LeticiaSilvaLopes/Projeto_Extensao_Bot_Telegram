...
bot.start(async (ctx) => 
{
    ...
    await ctx.reply(`Olá, ${nome}! O meu nome é Sophia e hoje estou aqui para lhe ajudar! Para começarmos, vou lhe passar todas as opções que temos, e peço para que selecione a desejada!
    Ah, e caso queira voltar ao menu inicial, é só escrever meu nome!`
   , primeiroMenu)
})

bot.hears(/sophia/i, async ctx => {
    const nome = ctx.update.message.from.first_name
    await ctx.reply(`Oie, ${nome}! Voltei! Vou lhe passar os menus novamente!`, primeiroMenu)
}) 

bot.hears('Games', async ctx => {
    await ctx.reply(`Escolha qual título você quer jogar`, 
    await Markup.keyboard(['Front-End', 'Back-End', 'Mobile', 'UX/UI', 'DevOps']).resize().oneTime().extra())
}) 

bot.hears('Jogar mais', async ctx => {
    await ctx.reply(`Escolha qual título você quer jogar.`, 
    await Markup.keyboard(['Front-End', 'Back-End', 'Mobile', 'UX/UI', 'DevOps']).resize().oneTime().extra())
})

const msgVoltarMenu = "Protinho, voltamos ao menu de início, o que deseja fazer agora?";

bot.hears('Voltar ao menu', async ctx => {
    await ctx.reply(msgVoltarMenu, primeiroMenu)   
})

bot.hears('Front-End', async ctx => {
    // await ctx.reply(gamesMenus)
}) 

bot.hears('Back-End', async ctx => {
    await ctx.reply(`Vamos ao jogo!`)
    await ctx.reply(`Back-End 1. O que são arrays?
    ...
    , await Markup.keyboard(['Back-End 1. A', 'Back-End 1. B', 'Back-End 1. C', 'Back-End 1. D']).resize().oneTime().extra())
}) 
...
bot.hears('Back-End 1. A', async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! E agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio2BKE, 
    await Markup.keyboard(['Back-End 2. A', 'Back-End 2. B', 'Back-End 2. C', 'Back-End 2. D']).resize().oneTime().extra())
}) 


bot.hears(['Back-End 1. B', 'Back-End 1. C', 'Back-End 1. D' ], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, até agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio2BKE, 
    await Markup.keyboard(['Back-End 2. A', 'Back-End 2. B', 'Back-End 2. C', 'Back-End 2. D']).resize().oneTime().extra())
}) 
...
bot.hears('Back-End 2. B', async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! E agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio3BKE, 
    await Markup.keyboard(['Back-End 3. A', 'Back-End 3. B', 'Back-End 3. C', 'Back-End 3. D']).resize().oneTime().extra())
}) 


bot.hears(['Back-End 2. A', 'Back-End 2. C', 'Back-End 2. D'], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, até agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio3BKE, 
    await Markup.keyboard(['Back-End 3. A', 'Back-End 3. B', 'Back-End 3. C', 'Back-End 3. D']).resize().oneTime().extra())
}) 
...
bot.hears(['Back-End 3. D'], async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! E agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio4BKE,
    await Markup.keyboard(['Back-End 4. A', 'Back-End 4. B', 'Back-End 4. C', 'Back-End 4. D']).resize().oneTime().extra())
}) 

bot.hears(['Back-End 3. A', 'Back-End 3. B', 'Back-End 3. C'], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, , até agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio4BKE,
    await Markup.keyboard(['Back-End 4. A', 'Back-End 4. B', 'Back-End 4. C', 'Back-End 4. D']).resize().oneTime().extra())
}) 
...
bot.hears(['Back-End 4. A'], async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! E agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio5BKE,
    await Markup.keyboard(['Back-End 5. A', 'Back-End 5. B', 'Back-End 5. C', 'Back-End 5. D']).resize().oneTime().extra())
}) 

bot.hears(['Back-End 4. B', 'Back-End 4. C', 'Back-End 4. D'], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, até agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio5BKE,
    await Markup.keyboard(['Back-End 5. A', 'Back-End 5. B', 'Back-End 5. C', 'Back-End 5. D']).resize().oneTime().extra())
}) 

bot.hears(['Back-End 5. C'], async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! Até agora você tem ${pontos}xp!.
    Agora que você finalizou o jogo de perguntas de Back-End, você deseja voltar ao menu inicial ou jogar mais?`, 
                    await Markup.keyboard(['Voltar ao menu', 'Jogar mais']).resize().oneTime().extra())
}) 

bot.hears(['Back-End 5. A', 'Back-End 5. B', 'Back-End 5. D'], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, mas até agora você acumulou ${pontos}xp.
    Agora que você finalizou o jogo de perguntas de Back-End, você deseja voltar ao menu inicial ou jogar mais?`, 
    await Markup.keyboard(['Voltar ao menu', 'Jogar mais']).resize().oneTime().extra())
}) 

bot.hears('UX-UI', async ctx => {
    // await ctx.reply(gamesMenus)
}) 

bot.hears('DevOps', async ctx => {
    await ctx.reply(`Vamos ao jogo!`)
    await ctx.reply(`DevOps 1. O que é DevOps?
    ...
    , await Markup.keyboard(['DevOps 1. A', 'DevOps 1. B', 'DevOps 1. C', 'DevOps 1. D']).resize().oneTime().extra())
}) 
...
bot.hears('DevOps 1. B', async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! E agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio2DOPs
    , await Markup.keyboard(['DevOps 2. A', 'DevOps 2. B', 'DevOps 2. C', 'DevOps 2. D']).resize().oneTime().extra())
}) 

bot.hears(['DevOps 1. A', 'DevOps 1. C', 'DevOps 1. D' ], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, até agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio2DOPs
    , await Markup.keyboard(['DevOps 2. A', 'DevOps 2. B', 'DevOps 2. C', 'DevOps 2. D']).resize().oneTime().extra())
}) 
...
bot.hears('DevOps 2. D', async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! E agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio3DOPs
    , await Markup.keyboard(['DevOps 3. A', 'DevOps 3. B', 'DevOps 3. C']).resize().oneTime().extra())
}) 


bot.hears(['DevOps 2. A', 'DevOps 2. B', 'DevOps 2. C' ], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, até agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio3DOPs
    , await Markup.keyboard(['DevOps 3. A', 'DevOps 3. B', 'DevOps 3. C']).resize().oneTime().extra())
}) 
...
bot.hears('DevOps 3. C', async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! E agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio4DOPs
    , await Markup.keyboard(['DevOps 4. A', 'DevOps 4. B', 'DevOps 4. C']).resize().oneTime().extra())
}) 


bot.hears(['DevOps 3. A', 'DevOps 3. B'], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, até agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio4DOPs
    , await Markup.keyboard(['DevOps 4. A', 'DevOps 4. B', 'DevOps 4. C']).resize().oneTime().extra())
}) 
...
bot.hears('DevOps 4. A', async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! E agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio5DOPs
    , await Markup.keyboard(['DevOps 5. A', 'DevOps 5. B', 'DevOps 5. C']).resize().oneTime().extra())
}) 


bot.hears(['DevOps 4. B', 'DevOps 4. C'], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, até agora você tem ${pontos}xp. Vamos a próxima questão!`)
    await ctx.reply(exercicio5DOPs
    , await Markup.keyboard(['DevOps 5. A', 'DevOps 5. B', 'DevOps 5. C']).resize().oneTime().extra())
}) 

bot.hears('DevOps 5. A', async ctx => {
    pontos += 400
    await ctx.reply(`Parabéns! Você acertou e ganhou 400xp! Até agora você tem ${pontos}xp!.
    Agora que você finalizou o jogo de perguntas de Back-End, você deseja voltar ao menu inicial ou jogar mais?`, 
                    await Markup.keyboard(['Voltar ao menu', 'Jogar mais']).resize().oneTime().extra())
...
bot.hears(['DevOps 5. B', 'DevOps 5. C'], async ctx => {
    await ctx.reply(`Infelizmente você errou a questão e não ganhou pontos, mas até agora você acumulou ${pontos}xp.
    Agora que você finalizou o jogo de perguntas de Back-End, você deseja voltar ao menu inicial ou jogar mais?`, 
    await Markup.keyboard(['Voltar ao menu', 'Jogar mais']).resize().oneTime().extra())
}) 
